...
class FourPaneApp(App):
    ...
    def _show_quit_dialog(self) -> None:
        """종료 확인 ModalScreen 표시"""
        self.push_screen(QuitConfirmScreen(), self._quit_callback)

    def _quit_callback(self, confirmed: bool | None) -> None:
        """종료 확인 콜백"""
        if confirmed:
            self.exit()

    def action_quit(self) -> None:
        """Textual 기본 Ctrl+Q 동작을 가로채서 종료 확인 팝업을 띄움"""
    ...
    def _animate_status(self, base_text: str) -> None:
        """LLM 동작중 애니메이션: ...을 1~5개로 변경 (안전하게 UI 호출)"""
        import time
        while True:
            for dots in [".", "..", "...", "....", "....."]:
    ...
    def _stop_llm_animation(self, status_text: str) -> None:
        """LLM 애니메이션 중지 후 상태 업데이트"""
        self.workers.cancel_group(self, "_animate_status")
        self._update_ui(status_text=status_text)
    ...
    def _worker_generate_plot_step1(self) -> None:
        ...
        try:
            ...
            self.call_from_thread(self._update_ui, editor_text=final_txt, status_text=f"상태: [{now}] Step1 완료 | 2번 메뉴로 Step2 실행", readonly=True)
            self.call_from_thread(self._focus_menu)

        except Exception as e:
            now = datetime.datetime.now().strftime("%H:%M:%S")
            self.call_from_thread(self._update_ui, editor_text=f"Step1 중 오류 발생:\n{e}", status_text=f"상태: [{now}] Step1 오류")
            self.call_from_thread(self._focus_menu)
    ...
    def _worker_generate_plot_step2(self) -> None:
        ...
        try:
            ...
            self.call_from_thread(self._update_ui, editor_text=debug_msg, readonly=True)
            now = datetime.datetime.now().strftime("%H:%M:%S")
            self.call_from_thread(self._stop_llm_animation, f"상태: [{now}] Step2 완료!")
            self.call_from_thread(self._focus_menu)

        except Exception as e:
            self.call_from_thread(self._update_ui, editor_text=f"Step2 중 오류 발생:\n{e}")
            now = datetime.datetime.now().strftime("%H:%M:%S")
            self.call_from_thread(self._stop_llm_animation, f"상태: [{now}] Step2 오류")
            self.call_from_thread(self._focus_menu)
    ...
    def _worker_generate_episode(self) -> None:
        ...
        try:
            ...
            if use_extended:
                ...
                self.call_from_thread(self._update_ui, editor_text=f"{plot_result}\n\n{prog_msg}", readonly=False)
                now = datetime.datetime.now().strftime("%H:%M:%S")
                self.call_from_thread(self._stop_llm_animation, f"상태: [{now}] 에피소드 생성 완료 (extended) | [E]: 편집 모드 토글")
            else:
                ...
                now = datetime.datetime.now().strftime("%H:%M:%S")
                self.call_from_thread(self._stop_llm_animation, f"상태: [{now}] 에피소드 생성 완료 | [E]: 편집 모드 토글")

        except Exception as e:
            self.call_from_thread(self._update_ui, editor_text=f"에피소드 생성 중 오류: {e}")
            self.call_from_thread(self._stop_llm_animation, "상태: 오류")
        self.call_from_thread(self._focus_menu)
    ...
    def _worker_generate_refine(self) -> None:
        ...
        try:
            ...
            self.call_from_thread(self._update_ui, editor_text=out_txt, readonly=False)
            now = datetime.datetime.now().strftime("%H:%M:%S")
            self.call_from_thread(self._stop_llm_animation, f"상태: [{now}] 생성 및 보완 완료 ({len(episodes)}개 에피소드) | [E]: 편집 모드 토글")
        except Exception as e:
            self.call_from_thread(self._update_ui, editor_text=f"생성 및 보완 중 오류: {e}")
            self.call_from_thread(self._stop_llm_animation, "상태: 오류")
        self.call_from_thread(self._focus_menu)
    ...
    def _worker_generate_story(self) -> None:
        ...
        try:
            ...
            self.call_from_thread(self._update_ui, editor_text=out_txt, readonly=False)
            now = datetime.datetime.now().strftime("%H:%M:%S")
            self.call_from_thread(self._stop_llm_animation, f"상태: [{now}] 스토리 생성 완료 | [E]: 편집 모드 토글")
        except Exception as e:
            self.call_from_thread(self._update_ui, editor_text=f"스토리 생성 중 오류: {e}")
            self.call_from_thread(self._stop_llm_animation, "상태: 오류")
        self.call_from_thread(self._focus_menu)
    ...
    def _worker_auto_run(self) -> None:
        ...
        try:
            ...
            if result["success"]:
                self.call_from_thread(self._update_ui, editor_text=f"전체 자동 실행 완료!\n\n{result['content_text']}", readonly=True)
                now = datetime.datetime.now().strftime("%H:%M:%S")
                self.call_from_thread(self._stop_llm_animation, f"상태: [{now}] 자동 실행 완료")
            else:
                self.call_from_thread(self._update_ui, editor_text=f"자동 실행 오류:\n{result['content_text']}")
                now = datetime.datetime.now().strftime("%H:%M:%S")
                self.call_from_thread(self._stop_llm_animation, f"상태: [{now}] 자동 실행 오류")
        except Exception as e:
            self.call_from_thread(self._update_ui, editor_text=f"자동 실행 중 오류: {e}")
            self.call_from_thread(self._stop_llm_animation, "상태: 오류")
        self.call_from_thread(self._focus_menu)
